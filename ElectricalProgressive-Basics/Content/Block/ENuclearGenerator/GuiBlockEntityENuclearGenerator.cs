...
public class GuiBlockEntityENuclearGenerator : GuiDialogBlockEntity
{
    private BlockEntityENuclearGenerator betestgen;
    private float _gentemp;


    public GuiBlockEntityENuclearGenerator(string dialogTitle, InventoryBase inventory, BlockPos blockEntityPos, ICoreClientAPI capi,
        BlockEntityENuclearGenerator bentity) : base(dialogTitle, inventory, blockEntityPos, capi)
    {
        if (base.IsDuplicate)
        {
            return;
    ...
    {
        this.capi.Event.EnqueueMainThreadTask(new Action(this.SetupDialog), "nucleargen");
    }
    ...
    public void SetupDialog()
    {
        ...
        this.SingleComposer = capi.Gui.CreateCompo("nucleargen" + (blockPos?.ToString()), window)
            .AddShadedDialogBG(dialog, true, 5)
            .AddDialogTitleBar(Lang.Get("nucleargen"), new Action(OnTitleBarClose), null, null)
