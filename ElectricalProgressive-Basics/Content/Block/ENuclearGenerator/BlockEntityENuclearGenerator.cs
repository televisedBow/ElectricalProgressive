...
namespace ElectricalProgressive.Content.Block.ENuclearGenerator;

public class BlockEntityENuclearGenerator : BlockEntityGenericTypedContainer, IHeatSource
{
    ...
    private InventoryNuclearGenerator _inventory;
    private GuiBlockEntityENuclearGenerator? _clientDialog;


    public float _genTemp = 20f;

    /// <summary>
    /// Кэш для мэша топлива, где int - размер топлива в генераторе (от 0 до 8)
    /// </summary>
    ...
    public override string DialogTitle => Lang.Get("nucleargen");

    public override string InventoryClassName => "nucleargen";

    public BlockEntityENuclearGenerator()
    {
        this._inventory = new InventoryNuclearGenerator(null!, null!);
    ...
    private void Calculate_kpd()
    {
        ...
        for (var level = 1; level <= 11; level++)
        {
            ...
            if (accessor.GetBlock(platePos) is not Termoplastini.BlockTermoplastini)
            {
    ...
    public override bool OnPlayerRightClick(IPlayer byPlayer, BlockSelection blockSel)
    {
        ...
        if (this.Api.Side == EnumAppSide.Client)
        {
            ...
            {
                this._clientDialog =
                    new GuiBlockEntityENuclearGenerator(DialogTitle, Inventory, this.Pos, this._capi!, this);
                _clientDialog.Update(_genTemp, _fuelBurnTime);
